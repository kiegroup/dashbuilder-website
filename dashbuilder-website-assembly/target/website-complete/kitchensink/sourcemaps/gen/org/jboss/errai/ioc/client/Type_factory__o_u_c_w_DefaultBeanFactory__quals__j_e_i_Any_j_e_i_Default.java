...
public class Type_factory__o_u_c_w_DefaultBeanFactory__quals__j_e_i_Any_j_e_i_Default extends Factory<DefaultBeanFactory> { private class Type_factory__o_u_c_w_DefaultBeanFactory__quals__j_e_i_Any_j_e_i_DefaultProxyImpl extends DefaultBeanFactory implements Proxy<DefaultBeanFactory> {
    ...
    @Override public WorkbenchPartPresenter newWorkbenchPart(String title, IsWidget titleDecoration, PartDefinition definition, Class partType) {
      if (proxyHelper != null) {
        final DefaultBeanFactory proxiedInstance = proxyHelper.getInstance(this);
        final WorkbenchPartPresenter retVal = proxiedInstance.newWorkbenchPart(title, titleDecoration, definition, partType);
        return retVal;
      } else {
        return super.newWorkbenchPart(title, titleDecoration, definition, partType);
      }
    }
