...
In the example below the dataset lookup gives the median of the number of products per section:
...
.Dataset Lookup with median
image::guides/yaml/median.png[Lookup with group]

In the example below the dataset lookup joins the number of products per section:
...
.Dataset Lookup with join
image::guides/yaml/join.png[Lookup with group]

In the example below the dataset lookup joins the number of products per section using comma:
...
.Dataset Lookup with join using comma
image::guides/yaml/joinComma.png[Lookup with group]

In the example below the dataset lookup joins the number of products per section using hyphen:
            ...
            group:
                - columnGroup:
                  ...
                  groupFunctions:
                    - source: Section
                    - source: Quantity
                      function: JOIN_HYPHEN
                      column: Total Products
----

.Dataset Lookup with join using hyphen
image::guides/yaml/joinHyphen.png[Lookup with group]

The supported group functions are `SUM`, `MAX`, `MIN` , `AVERAGE` and `MEDIAN` for numbers. For label columns the supported functions are `DISTINCT` and `COUNT`.  
...
New group functions are as follows:-
...
Displayers have the following new fields:
...
There are new fields which can be added to Global properties; they are as  folows:-
